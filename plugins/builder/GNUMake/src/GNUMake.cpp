...
bool GNUMake::setEnabled( bool b)
{
 	...
 	if ( b )
	{
		...
		foreach ( pCommand c, pCommandList() << buildCommand() << userCommands() )
		{
...
pCommand GNUMake::buildCommand() const
{
	...
	QSettings* s = settings();
	pCommand c;
	c.setText( s->value( "BuildCommand/Text" ).toString() );
	c.setCommand( s->value( "BuildCommand/Command" ).toString() );
	c.setArguments( s->value( "BuildCommand/Arguments" ).toString() );
...
{
	QSettings* s = settings();
	s->setValue( "BuildCommand/Text", c.text() );
	s->setValue( "BuildCommand/Command", c.command() );
	s->setValue( "BuildCommand/Arguments", c.arguments() );
