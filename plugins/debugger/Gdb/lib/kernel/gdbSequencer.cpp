...
void GdbSequencer::start(const int & i)
{
	...
	{
		GdbCore::Parser()->setNextCommand(mClassName, mCmdList.at(i).cmd );
		GdbCore::Process()->sendRawData( mCmdList.at(i).cmd );
	}
}
