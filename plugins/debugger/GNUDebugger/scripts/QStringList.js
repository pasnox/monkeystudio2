...
<unsigned>
	...
	size = p varname.d.alloc
	addr = p &varname.d.array
	x /size(type) addr

QStringList

	numElement = p varname.d.end
	ou
	numElement = p varname.d.alloc - 1

	addrStore = p &varname.d.array

	list des addr de tout les elements

	addrElements = x /numElement(x) addr

	ou just un element

	addrStore = p varname.d.array[0]

	sizeElement = p ((QString)addrElements).d.alloc
	Element = x /sizeElement(s) addrElements		
...
send = function(parent , varName , isPointer , sequenceNumber)
{
	...
	switch(sequenceNumber)
	{
		...
		// x /3x 0x3Df56 , adresse des elements de la list
//		case 2 : return sendToGdb(parent , "x /1x " + listAddr);
