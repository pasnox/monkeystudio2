#ifndef ABSTRACTPROJECTMODEL_H
#define ABSTRACTPROJECTMODEL_H
//
...
typedef QHash<int, AbstractProjectModel*> QHashProjects; // id, project
//
class Q_MONKEY_EXPORT AbstractProjectModel : public QAbstractItemModel
{
	Q_OBJECT
	Q_ENUMS( NodeRole NodeType )
	friend class AbstractProjectProxy;
	//
public:
	enum NodeRole
	{
	...
	//
	AbstractProjectModel( const QString& = QString::null, QObject* = 0 );
	virtual ~AbstractProjectModel();
	// project id
	int id() const;
	// get project by id
	static AbstractProjectModel* byId( int );
