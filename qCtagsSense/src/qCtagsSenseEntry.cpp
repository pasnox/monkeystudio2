...
qCtagsSenseEntry::qCtagsSenseEntry( qCtagsSenseEntry* _p, bool _lne, ulong _ln,  bool _fs, bool _fe, bool _tl, qCtagsSenseEntry::Kind _k )
{
	parent = _p;
...
qCtagsSenseEntry::qCtagsSenseEntry( const QSqlRecord& record, const QString& _fileName )
{
	fileName = _fileName.isEmpty() ? record.value( "fileName" ).toString() : _fileName;
	language = record.value( "language" ).toString();
	...
	kind = (qCtagsSenseEntry::Kind)record.value( "kind" ).toInt();
	// extended fields
...
qCtagsSenseEntry::~qCtagsSenseEntry()
{
	qDeleteAll( children );
}
